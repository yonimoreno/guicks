...
window.GI = {
  ...
  async load(cb){
    ...
    // ── Try new per-app structure: lang/core/XX.js + lang/slug/XX.js ──
    if(slug){
      const coreOk=await this._loadScript(base+'lang/core/'+code+'.js');
      if(coreOk){
        this._flush(merged);
        const appOk=await this._loadScript(base+'lang/'+slug+'/'+code+'.js');
        if(appOk) this._flush(merged);
        // If no app file yet, top up from old flat file
        if(!appOk){
          await this._loadScript(base+'lang/guicks-lang-'+code+'.js');
          if(window.__GI_LANG__){ Object.assign(merged,window.__GI_LANG__); window.__GI_LANG__=null; }
        }
        window.__GI_CACHE__[cacheKey]=merged;
        this._t=merged; if(cb)cb(); return;
      }
    }

    // ── Fallback: old flat lang/guicks-lang-XX.js ──────────────────
    if(window.__GI_CACHE__[code]){
      this._t=window.__GI_CACHE__[code]; if(cb)cb(); return;
    }
    const ok=await this._loadScript(base+'lang/guicks-lang-'+code+'.js');
    if(ok&&window.__GI_LANG__){
      window.__GI_CACHE__[code]=window.__GI_LANG__;
      this._t=window.__GI_LANG__;
      window.__GI_LANG__=null;
    } else if(!ok&&code!=='en'){
      await this._loadScript(base+'lang/guicks-lang-en.js');
      if(window.__GI_LANG__){ this._t=window.__GI_LANG__; window.__GI_LANG__=null; }
    }
    if(cb)cb();
  },
